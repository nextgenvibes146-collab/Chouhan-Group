...
const UserMenu: React.FC<{ user: User, onLogout: () => void }> = ({ user, onLogout }) => {
    ...
    return (
        <div className="relative" ref={menuRef}>
            <button onClick={() => setIsOpen(!isOpen)} className="flex items-center space-x-2 p-1 rounded-full hover:bg-gray-100">
                <img 
                    src={user.avatarUrl}
                ...
                <svg className={`hidden md:block w-4 h-4 text-text-secondary transition-transform ${isOpen ? 'rotate-180' : ''}`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
            </button>
            {isOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-surface rounded-lg shadow-lg border border-border-color z-20">
                    <div className="p-2">
                        <a href="#" onClick={(e) => { e.preventDefault(); onLogout(); }} className="block w-full text-left px-4 py-2 text-sm text-text-primary rounded-md hover:bg-gray-100">
                            Logout
                        </a>
                    </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md m-4" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-4 bg-gray-50 flex justify-end space-x-2">
                    ...
                    <button onClick={handleSend} className="px-4 py-2 text-sm font-medium text-white bg-primary rounded-md hover:bg-primary-hover">Send</button>
                </div>
            </div>
        </div>
    );
...
const Header: React.FC<HeaderProps> = ({ searchTerm, onSearchChange, searchResults, users, currentUser, onLogout, onRefresh, onToggleSidebar, notifications, onAddNotification, onMarkNotificationsAsRead }) => {
  ...
  return (
    <header className="flex items-center justify-between h-20 px-4 md:px-6 bg-surface border-b border-border-color flex-shrink-0">
      <div className="flex items-center">
         <button onClick={onToggleSidebar} className="md:hidden mr-4 p-2 text-text-secondary rounded-full hover:bg-gray-100">
      ...
      <div className="flex items-center space-x-2 md:space-x-4">
        {currentUser.role === 'Admin' && (
            <button onClick={() => setBroadcastModalOpen(true)} className="px-3 py-2 text-sm font-medium text-white bg-primary rounded-md hover:bg-primary-hover hidden sm:block">
                Broadcast
            </button>
        )}
        <button
          onClick={onRefresh}
          className="px-3 py-2 text-sm font-medium text-text-secondary bg-gray-100 rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary hidden sm:block"
        >
          Refresh
        </button>
        <div className="relative">
            <button onClick={handleToggleNotifications} className="p-2 text-text-secondary rounded-full hover:bg-gray-100 hover:text-text-primary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
            ...
            {isNotificationsOpen && (
                <div className="absolute top-full right-0 mt-2 w-80 bg-surface border border-border-color rounded-lg shadow-lg z-20">
                    ...
                    <ul className="max-h-96 overflow-y-auto">
                        {userNotifications.length > 0 ? userNotifications.map(n => (
                            <li key={n.id} className={`p-3 border-b border-border-color last:border-b-0 ${!n.isRead ? 'bg-blue-50' : ''}`}>
                                ...
                                <p className="text-xs text-text-secondary mt-1">{new Date(n.createdDate).toLocaleString()}</p>
                            </li>
                        )) : <li className="p-4 text-center text-sm text-text-secondary">No new notifications.</li>}
                    </ul>
                </div>
            )}
        </div>
